...
(async () => {
  try {
    const { data, error } = await supabase.from("guilds").select("discord_id");
    ...
    if (data) {
      const promises = data.map(async (guild) => {
        try {
          ...
          functions.logWithTimestamp(
            `Server '${fetchedGuild.name}': ${commands.length} Slash-Commands erfolgreich neu geladen.`
          );
        } catch (guildError) {
          console.error(
            `Fehler beim Aktualisieren von Befehlen für Guild ID ${guild.discord_id}:`,
            guildError
          );
        }
      });
